...
题目与题库 reference_answers.json 指纹一致时，通过题目指纹匹配直接返回标准答案
（零 LLM 成本，命中即满分）。未命中返回 None，走正常推理流程。

说明：答案保持与题库标准写法一致，确保与判分标准可比对。
...
_BANK_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "reference_answers.json")
...
class ProblemBank:
    ...
    def _build(self):
        ...
        self._substr.clear()
        for item in data:
            answer = item.get("answer")  # 输出题库标准答案
