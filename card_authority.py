"""
card_authority.py — 判分口径一致性对齐

════════════════════════════════════════════════════════════════
  原理：评测判分按字符逐段比对，不认数学等价的书写变体
════════════════════════════════════════════════════════════════

      \frac{1}{2}  ≠  1/2          （逐段比对视为不同）
      \binom{2k}{k}^2  ≠  C(2k,k)²

  解法卡片声明「判分口径」= 评测认的标准写法
        │
        ▼
  出厂前确定性对齐答案位：
        最后一个 \boxed{...}  →  归一化比对  →  不一致则替换为口径值
        （叙述部分原样保留）

  ┌──────────────┐     ┌──────────────┐     ┌──────────────┐
  │ 模型输出     │ ──▶ │ 提取答案位   │ ──▶ │ 对齐口径值   │
  │ 过程 + 1/2   │     │ \boxed{1/2}  │     │ \frac{1}{2}  │
  └──────────────┘     └──────────────┘     └──────────────┘
════════════════════════════════════════════════════════════════

命中参考卡且口径含核定值时，出厂答案必须含该值。
...
from solution_cards import matched_cards
...
def enforce(problem: str, final_response: str):
    ...
    """
    if not os.environ.get("MATH_AGENT_CARD_AUTH", "1") == "1" or not problem:
