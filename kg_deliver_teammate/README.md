...
| 文件/目录 | 作用 |
|---|---|
| `knowledge_graph.py` | 解题过程图谱（18 题型方法论本体） |
| `theorem_kg.py` + `theorem_data.py` | 定理图谱（160 实体 + 14 同名根消歧 + 证据链） |
| `kg_combined.py` | 模板+定理+证据链三合一联合注入入口 |
| `kg_fast_retriever.py` | **一行调用的快速检索入口**：`retrieve_experience(problem, domain)` 返回解题经验提示 |
| `kg_retriever.py` | **统一检索器（新增）**：`unified_retrieve(problem)` 自动判题型并聚合题型方法论/命中定理/模板—定理图经验三路；CLI 一键查询（`python kg_retriever.py "题目"`） |
| `pipeline/kg_runtime_graph.py` | 模板—定理图谱高效检索器（域预过滤 + 邻接表 + Algorithm 1） |
| `pipeline/artifacts/` | **图谱交付物**：`template_theorem_graph.json`（639 模板/62 定理/323 边）+ `templates_emb.npy`（564×512 向量，2026-09-14 换源重建） |
| `pipeline/kg_theorem_extract.py` | 定理抽取（theorem_kg 运行依赖） |
| `kg_bridge_example.py` | 题型分类接入桥示例 |
| `kg_domain_summary.json` | 领域方法摘要数据 |
| `usage_example.py` | 最小使用示例 |

> **只需拿 `kg_deliver` 目录**；嵌入模型 bge-small-zh-v1.5 需自行准备，
> 改 `pipeline/kg_runtime_graph.py` 顶部 `MODEL_DIR`（或设环境变量 `KG_BGE_MODEL_DIR`）指向本机路径。
...
下图谱已配齐 18 个题型的真实解题过程（每条含题干 + 完整解答），
**2026-09-14 按用户决策完全换源**：清空旧源，当前共 **13727 条**，主体为 8 类权威新源
（Omni-MATH / MATH / BlueMO / odyssey / INEQMATH / ProofNet / APMO / Putnam），
归桶策略：源自带分类映射 → 英文关键词 → 中文关键词（BlueMO）。数据源说明如下：

| 数据源 | 原始仓库/存档 | 覆盖题型 | 说明 |
|---|---|---|---|
| **Omni-MATH**（主源） | `github.com/KbsdJames/Omni-MATH` | 18 域（自带 domain 树映射） | 4428 条，题+解+答，带难度评分 |
| **MATH (testing)** | `github.com/NolanTrem/MATH-Scraper` | 代数/几何/数论/组合/微积分 | `MATH_dataset_full_testing.csv` 5000 条，自带 type 学科 |
| **BlueMO** | `github.com/yifanzhang-pro/BlueMO` | 18 域（中文关键词归桶） | 3352 条中文奥赛难题，题+解 |
| **odyssey-math** | `github.com/protagolabs/odyssey-math` | 多域（自带 label 映射） | 387 条，题+推导+答 |
| **ProofNet** | `github.com/zhangir-azerbayev/ProofNet` | 高数域（关键词归桶） | 371 条大学数学证明（`nl_statement`+`nl_proof`） |
| **Putnam** | `github.com/rpxgit/The-Putnam-Archive` | 多域 | 372 条官方 Archive（导入 348，24 条跨源去重） |
| **APMO** | `apmo-official.org` | 多域 | 155 条；**官方解缺失 142 条已跳过**，导入 13 条 |
| **INEQMATH** | `github.com/lupantech/ineqmath` | 不等式专题→algebra | 200 条（导入 187），solution 为 list 已拼接 |
