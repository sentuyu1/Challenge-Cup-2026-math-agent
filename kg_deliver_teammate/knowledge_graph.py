...
__all__ = [
    ...
    "kg_template_theorem_block", "kg_template_graph_block",
]
...
_SUMMARY_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "kg_domain_summary.json")
_summary_cache: dict = {}


def _load_domain_summaries() -> dict:
    """惰性加载领域方法摘要（kg_domain_summary.py 生成）；缺失/坏文件返回空 dict。"""
    global _summary_cache
    if _summary_cache:
        return _summary_cache
    try:
        with open(_SUMMARY_FILE, encoding="utf-8") as f:
            _summary_cache = json.load(f).get("domains", {})
    except Exception:  # noqa: BLE001
        _summary_cache = {}
    return _summary_cache


def _aimo_ref_block(n: int = 2) -> str:
    """加载兜底桶题解前 n 条作为"综合竞赛参考"（未细分题型）。

    这些竞赛题未归入 18 题型，但对数学解题是通用参考；注入时放在方法论之后、
    长度受限，减少噪声。文件缺失/坏文件返回 ""。
    """
    try:
        fp = os.path.join(_KG_CACHE_DIR, "aimo_综合.json")
        ...
        for it in items:
            ...
            m = str(it.get("method") or "")[:60]
            lines.append(f"- {t}" + (f"：{m}…" if m else ""))
        return "\n".join(lines)
    except Exception:  # noqa: BLE001
        return ""
...
def retrieve_kg(domain: str, problem: str = "", use_extension: bool = True,
                max_len: int = 1400, use_summary: bool = True,
                use_aimo_ref: bool = True) -> str:
    """按题型取知识图谱节点，组装为中文方法论提示文本。

    - 按 domain_en 精确取节点组装提示；
    - use_summary=True（默认）时追加"领域方法摘要"（kg_domain_summary.json 规则聚合，
      缺失则静默跳过），TagRAG 摘要融合思想；
    - use_aimo_ref=True（默认）时追加"综合竞赛参考"（兜底桶题解前 2 条，
      未归桶竞赛题兜底）；
    ...
    try:
        ...
        budget = (max(100, max_len // 6) if use_summary else 0) \
            + (120 if use_aimo_ref else 0)  # 摘要 + 兜底桶预留
        text = _assemble(node, max_len=max_len - budget)
        if use_summary and max_len // 6:
            try:
                sums = _load_domain_summaries().get(domain)
                if sums:
                    block = "\n\n## 领域方法摘要（题解聚合）\n" + "\n".join(
                        f"- {s[:40]}" for s in sums[:6])
                    if len(text) + len(block) <= max_len:
                        text += block
            except Exception:  # noqa: BLE001 - 摘要缺失/坏文件不影响主输出
                pass
        if use_aimo_ref:
            try:
                block = _aimo_ref_block()
                if block:
                    text += block
                    if len(text) > max_len:
                        text = text[:max_len] + "\n…(已截断)"
...
# ===================== 模板-定理桥（Template-Theorems 论文落地）=====================
# 论文结论：模板(解题套路)与定理**联合注入**优于单用（MATH 89.2→91.4，AMC10 73.27→77.88）。
# 本桥把"题型方法模板"与"对应定理的数学表述"拼成一段联合提示，供检索后注入。
# 设计约束：保持零依赖——theorem_kg 不可用（import 失败）时返回空串，绝不抛异常。

def kg_template_theorem_block(domain: str, max_len: int = 700) -> str:
    """返回"题型方法模板 → 对应定理"的联合提示段；无节点/模块不可用返回 ""。

    - 模板部分：该题型的核心方法（methods）与标准流程（flow）摘要；
    - 定理部分：从 node.theorems 提取定理名，经 theorem_kg.find_theorems 取
      statement 拼成"定理名：表述"。
    - 不改变 retrieve_kg 默认行为；此函数供联合注入入口（kg_combined）调用。
    """
    try:
        if not domain:
            return ""
        ...
        if node is None:
            return ""
        lines = [f"\n\n【模板-定理桥：{node.domain_cn}（{node.domain_en}）】"]
        # 1) 模板部分：方法 + 流程
        tmpl = []
        tmpl += [f"- {m}" for m in node.methods[:4]]
        if node.flow:
            tmpl.append("流程：" + " → ".join(f.split(". ", 1)[-1] for f in node.flow[:4]))
        if tmpl:
            lines.append("## 解题模板")
            lines += tmpl
        # 2) 定理部分：node.theorems → find_theorems → statement
        try:
            from theorem_kg import find_theorems  # 惰性导入，缺模块返回空
            th_lines = []
            for t in node.theorems[:6]:
                name = t.split("：")[0].strip() if "：" in t else t.strip()
                if not name:
                    continue
                hits = find_theorems(name, domain)
                if hits:
                    st = (hits[0].statement or "").strip()
                    th_lines.append(f"- {hits[0].theorem_cn}：{st[:120]}" if st
                                    else f"- {hits[0].theorem_cn}")
            if th_lines:
                lines.append("## 关联定理（数学表述）")
                lines += th_lines
        except Exception:  # noqa: BLE001 - 保持零依赖，theorem_kg 缺失则仅给模板
            pass
        text = "\n".join(lines)
        if len(text) > max_len:
            text = text[:max_len] + "\n…(已截断)"
...
def kg_template_graph_block(domain: str, problem: str = "", max_len: int = 600) -> str:
    """论文"模板—定理图谱"检索段注入（AAAI-26 图检索增强，规则近似）。

    - 检索：以 problem 在 pipeline.kg_runtime_graph 的模板—定理图谱中做
      Algorithm 1 检索（top_k=5, 阈值 m=0.45），拼"模板 + 定理表 + 回选模板"。
    - 开关：环境变量 MATH_AGENT_TEMPLATE_GRAPH（默认 0=关），避免改变现有
      retrieve_kg / kg_template_theorem_block 默认行为。
    - 降级：pipeline 模块/产物缺失时返回 ""，绝不抛异常（零依赖设计）。
    """
    try:
        if os.environ.get("MATH_AGENT_TEMPLATE_GRAPH", "0") != "1":
            return ""
        if not problem:
            return ""
        import sys as _sys
        _pipe = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pipeline")
        if _pipe not in _sys.path:
            _sys.path.insert(0, _pipe)
        from kg_runtime_graph import graph_retrieve_block
        # domain 非空时走高效域过滤检索（识别题型→在该域模板子集内检索）
