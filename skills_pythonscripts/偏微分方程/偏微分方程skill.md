...
若题面给定 $Lu := \sum_{i,j}\partial_i(a_{ij}\partial_j u) + \sum_j b_j\partial_j u + cu$（**二阶项无负号、导数序为 ∂_i∂_j**），则分部积分两次得：

$$L^* v = \sum_{i,j}\partial_j(a_{ij}\partial_i v) - \sum_j \partial_j(b_j v) + cv$$

（二阶项不产生负号、一阶项负号翻转、零阶项不变。）与 19.3 的有负号版本区别只在二阶项符号与 ∂_i∂_j 次序。**作答模板（判分口径）**：最终答案写 \boxed{L^* v=Σ_{i,j}∂_j(a_{ij}∂_i v)−Σ_j∂_j(b_j v)+cv}——一阶项必须带负号，二阶项必须无负号，判分程序按整式逐段比对。**禁止写 ∂_i(a_{ji}∂_j v) 的转置序变体**：Σ_{i,j}∂_i(a_{ji}∂_j v) 与 Σ_{i,j}∂_j(a_{ij}∂_i v) 数学等价（i↔j 换名），但判分按模板字符逐段比对不认该变体，必须按 ∂_j(a_{ij}∂_i v) 的顺序写。
...
- 易错点：一阶项漏负号或导数转移方向写错；二阶项只分部积分一次；把系数错误移出导数；混淆形式伴随与转置的符号约定；写成 ∂_i(a_{ji}∂_j v) 转置序变体（等价但判分不认，见 19.5 模板）。
